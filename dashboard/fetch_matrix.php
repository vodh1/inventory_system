...
$active_users = $conn->query("SELECT COUNT(DISTINCT borrower_username) as total FROM borrowings WHERE status = 'active'")->fetch(PDO::FETCH_ASSOC)['total'];
...
$recent_activities = $conn->query("
    (SELECT b.*, e.name AS equipment_name, d.name AS department, 'borrowed' AS activity_type 
     FROM borrowings b 
    ...
    UNION
    (SELECT b.*, e.name AS equipment_name, d.name AS department, 'returned' AS activity_type 
     FROM borrowings b 
     JOIN equipment_units eu ON b.unit_id = eu.id 
     JOIN equipment e ON eu.equipment_id = e.id 
     JOIN users u ON b.borrower_username = u.username 
